...
import { getFeedbacks } from "../../redux/features/feedbackSlice";
...
export default function TestimonialSlider() {
  ...
  if (isLoading) {
    content = <p className="mt-4 text-gray-600 md:text-lg text-center">Loading...</p>;
  }

  if (!isLoading && feedbacks?.length === 0) {
    content = <p className="mt-4 text-gray-600 md:text-lg text-center">No feedbacks yet.</p>;
  }

  if (!isLoading && feedbacks?.length > 0) {
    content = <div className="relative w-full mx-auto px-2 md:px-12">
    <div className="overflow-hidden">
      ...
      >
        {feedbacks?.map(({_id, image, name, position, quote}) => (
          <div
            key={_id}
